...
const ComoFunciona = () => (
  <section className="w-full max-w-5xl mx-auto py-20 px-4 bg-[#ff4a4a]">
    <h2 className="text-3xl font-black mb-12 text-center tracking-tight text-white">
      Como funciona <span className="text-white">na prática?</span>
    </h2>
    <div className="flex flex-col md:flex-row gap-10 md:gap-8 md:justify-between">
      {etapas.map((e, i) => (
        <div
          key={e.titulo}
          className="flex-1 bg-white/10 border border-white/20 rounded-3xl shadow-2xl p-10 flex flex-col items-center group hover:scale-105 hover:shadow-white/20 transition-transform duration-300 animate-fade-in backdrop-blur-sm"
        ...
        >
          ...
          <p className="mt-3 text-base text-white text-center font-medium">{e.desc}</p>
        </div>
      ))}
    </div>
  </section>
