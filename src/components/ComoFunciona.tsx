...
const etapas = [
  {
    ...
    icone: <Target size={44} className="text-[#ff9400] drop-shadow-lg bg-white/70 rounded-full p-2" />
  },
  {
    ...
    icone: <Clapperboard size={42} className="text-[#ff9400] drop-shadow-lg bg-white/70 rounded-full p-2" />
  },
  {
    ...
    icone: <Rocket size={44} className="text-[#ff9400] drop-shadow-lg bg-white/70 rounded-full p-2" />
  }
];
...
const ComoFunciona = () => (
  <section className="w-full max-w-5xl mx-auto py-20 px-4">
    <h2 className="text-3xl font-black mb-12 text-center tracking-tight">
      Como funciona <span className="text-[#ff9400]">na prática?</span>
    </h2>
    <div className="flex flex-col md:flex-row gap-10 md:gap-8 md:justify-between">
      {etapas.map((e, i) => (
        <div
          key={e.titulo}
          className="flex-1 bg-white border border-[#fff0ea] rounded-3xl shadow-2xl p-10 flex flex-col items-center group hover:scale-105 hover:shadow-[#ff940033] transition-transform duration-300 animate-fade-in"
          style={{ animationDelay: `${i * 80}ms` }}
        >
          <div className="mb-3 group-hover:scale-125 transition-transform">{e.icone}</div>
          <h3 className="mt-1 text-xl font-bold text-[#ff9400] text-center">{e.titulo}</h3>
