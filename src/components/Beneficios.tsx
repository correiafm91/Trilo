...
const beneficios = [
  {
    ...
    icone: <Award size={36} className="text-[#ff9400] drop-shadow-md bg-white/60 rounded-full p-1.5" />
  },
  {
    titulo: "Novas Fontes de Renda",
    desc: "Cursos online oferecem receita escalável, previsível e ampliam seu impacto.",
    icone: <Coins size={34} className="text-[#ff9400] drop-shadow-md bg-white/60 rounded-full p-1.5" />
  },
  {
    titulo: "Zero Complexidade Técnica",
    desc: "A Trilo resolve tudo: tecnologia, edição, plataforma e atendimento ao aluno.",
    icone: <ShieldCheck size={36} className="text-[#ff9400] drop-shadow-md bg-white/60 rounded-full p-1.5" />
  }
];
...
const Beneficios = () => (
  <section className="w-full max-w-5xl mx-auto py-16 px-4">
    <h2 className="text-3xl font-black mb-10 text-center tracking-tight">
      Por que escolher a <span className="text-[#ff9400]">Trilo?</span>
    </h2>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
      {beneficios.map((b, i) => (
        ...
        >
          ...
          <h3 className="mt-2 text-lg font-bold text-[#ff9400] text-center">{b.titulo}</h3>
